...
**box plot shows the relationship between Species and
flipper\_length\_mm. On average the flipper\_length\_mm for Gentoo is
greater then the other two species**:
...
**jitterplot has distributed the samples based on the
flipper\_length\_mm. Different clusters can been seen from the plot
above.**:
